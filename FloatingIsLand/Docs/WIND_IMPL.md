...
| `Building` | windScoreByLevel 等曲线列 | 各建筑的风力收益/惩罚曲线（风帆/船坞/风车有收益曲线，居民区有穿风惩罚，风向标有居民区倍率） |
...
- **被摆建筑自己身上的飘字**（§7.4 预览与落地共用）：顶行「周边合计 ±N」= 范围内所有已计入加减分的净和（邻居 + 元素 + 风，不含基础分/孤立惩罚这类 Self 条目）；建在风带上时，下面逐条列风的加减分（风力即时分 / 强风穿过 / 物流风覆盖），用与流线同色系的字。实现见 `ScoreHighlightPresenter.EmitSelfLabels`。
- **风变更一次性分也进预览**：摆风帆/物流点悬停时，对假设风场干跑 `WindScoreKeeper.PreviewWindChange`（**只读不落账**，预演不消耗一生一次额度），把「摆下去会给谁发分」（物流风覆盖 +8 / 互联 +30）飘到受益建筑头上；正在摆的假设物流点以 Id=-1 参与互联配对，「新点连老点」预览可见。预览数字与落地入账严格同源。注意：普通风（非物流风）经过已落地建筑不产生分，这是定稿规则——预览没显示分的建筑就是真的不会得分。
