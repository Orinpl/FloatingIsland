...
当前标定结果（stage_1，30 局）：平均到达 **19.1 / 20** 级，通关率 **87%**，
总支出占总收入 **84.7%**，单栋均分从 37 成长到 ~79；前期覆盖率 1.2~2.4（学习期宽松），
后期 L14、L16~L19 覆盖率 0.82~0.97 需要吃老本（设计要的压力区）。
贪心 AI 比真人强得多，所以这个数对真人而言已经是有真实失败风险的强度；
再往下调需要真实试玩数据，而不是继续跑仿真。

> **改了 `BuildingGroupTheme` 就必须重跑一次定价。** 分组换了收入曲线就跟着换——
> 主题化改版前后同一套 `unlockCost` 的支出占比从 85% 漂到 90%，通关率从 83% 掉到 57%。
> 顺序照旧不能反：先 `--free-unlock` 量收入，再按「总支出 ≈ 总收入 × 85%」定价。

仿真报告还带两张验收表，专门用来看分组本身对不对：

- **每级选中的主题分布**：验收阶段节奏（前期只该出生产线，L11 起才该出风能/物流/港口）；
- **建筑出现量**：验收组内配比与「放不下」的风险。当前 30 局每局落地
  采矿站 8.7 / 工坊 4.6、农田 11.6 / 粮仓 3.5 / 风车 2.5、居民区 28.5 / 市民中心 1.8（硬上限 3）、
  物流点 2.3 / 物流中心 0.4、风帆 2.3、船坞 0.8。跳过只有风帆 6 次与市民中心 1 次
  （全部 30 局合计）——风帆必须落在风带上、市民中心占地 4×4，这两栏是「放不下」的主要风险源。
